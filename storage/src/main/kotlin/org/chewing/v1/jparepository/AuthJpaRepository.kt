...
interface AuthJpaRepository : JpaRepository<AuthJpaEntity, String> {
    @Query("SELECT a FROM AuthJpaEntity a JOIN FETCH a.email e Join Fetch a.user WHERE e.emailAddress = :email")
    fun findUserByEmail(
    ...
    @Query("SELECT a FROM ConfirmEmailJpaEntity a JOIN FETCH a.email e WHERE e.emailAddress = :email AND a.authorizedNumber = :verificationCode")
