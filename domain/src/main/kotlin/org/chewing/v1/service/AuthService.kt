...
class AuthService(
    ...
    private val oauth2Client: OAuth2ClientService, // 추가된 OAuth2 클라이언트 서비스
    private val fileProcessor: FileProcessor,     // 프로필 사진
) {
    ...
    fun signUpWithEmail(
        ...
        device: PushToken.Device,
        appToken: String,
        profileImage: MultipartFile? // 프로필 이미지 처리 추가
    ): Pair<String, String> {
        ...
        // 프로필 이미지가 있으면 처리
        profileImage?.let {
            val file = FileUtil.convertMultipartFileToFile(it)
            val media = fileProcessor.processNewFile(user.userId, file)
            userAppender.updateProfileImage(user, media)
        }
    ...
    fun signUpWithGoogle(
        oauthToken: String,
        profileImage: MultipartFile? // 프로필 이미지를 선택적으로 처리
    ): Pair<String, String> {
        val googleUserInfo = oauth2Client.getGoogleUserInfo(oauthToken)
        val user = userAppender.appendUser(
            UserContent.of(googleUserInfo.name, googleUserInfo.email, googleUserInfo.birth)
        )
        ...
        val auth = authAppender.appendAuthInfoByEmailId(googleUserInfo.email, user.userId)
        return makeTokenPair(user.userId, auth)
    }
    ...
    fun signUpWithApple(
        ...
        val appleUserInfo = oauth2Client.getAppleUserInfo(oauthToken)
        val user = userAppender.appendUser(
            UserContent.of(appleUserInfo.name, appleUserInfo.email, appleUserInfo.birth)
        )
        ...
        val auth = authAppender.appendAuthInfoByEmailId(appleUserInfo.email, user.userId)
        return makeTokenPair(user.userId, auth)
    }
